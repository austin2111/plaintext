...
void CBlowFish::dumpPArray() {
    int i;
    uint32_t *PArrayAs32 = (void *)PArray;
cout << endl << "PArray: ";

    for(i = 0; i < 18; ++i)
        printf("%08x", PArrayAs32[i]);
...
void CBlowFish::Blowfish_decipher (DWORD *xl, DWORD *xr)
{
   union aword  Xl ;
   union aword  Xr ;
   // Remove ntohl if this doesn't work
   Xl.dword = ntohl(*xl) ;
   Xr.dword = ntohl(*xr) ;
...
BYTE            key[] = "IperaKey",
        *plain = strdup("test12"),
        ...
        int lcount;
        for (lcount = 1; lcount < argc; ++lcount) 
            sscanf(argv[lcount], "%x", &encode[lcount-1]);

        
        /******************************/

int     elen;
    unsigned char fubar = 0;

    do {
        plain[6] = fubar;
        cbf.Initialize(key, sizeof(key) - 1);
    } while (fubar++ != 0xff);
        cbf.Decode(encode, decode, argc-1);

        for (lcount = 0; lcount < argc - 1; lcount += 4) {
            swap(decode[lcount], decode[lcount+3]);
            swap(decode[lcount+1], decode[lcount+2]);
         }
        printf("\ndecoded password: ");
        for (lcount = 0; lcount < argc - 1; ++lcount)
            printf("%c", decode[lcount]);
        printf("\n");
        return 0;
}
